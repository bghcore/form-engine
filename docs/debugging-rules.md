...
When a single field value changes at runtime, the `processBusinessRule()` method in `BusinessRulesProvider` executes this pipeline:
...
| 2. **Revert previous rules** | `RevertFieldBusinessRule()` | Resets dependent fields to their default config state (component, required, hidden, readOnly, validations, valueFunction, confirmInput). Dropdown options are preserved from the current business rules since they may have been set by other sources. |
...
| 4. **Apply new value rules** | `ProcessFieldBusinessRule()` | Looks up the field's `dependencies` config, finds the entry matching the new value, and applies the resulting rule overrides to each dependent field. |
| 5. **Evaluate combo (AND) rules** | `ProcessComboFieldBusinessRule()` | For each field in the changed field's `comboDependentFields`, checks whether ALL conditions in its `dependencyRules.rules` are met. If all match, applies `updatedConfig`. If any fail, reverts to default. |
...
1. **Dependency override** (highest) -- The value from `IFieldConfig.dependencies[value][field]`
...
| `previousState` | `Partial<IBusinessRule>` | The affected field's state before this rule was applied. |
| `newState` | `Partial<IBusinessRule>` | The affected field's state after this rule was applied. |

### Trace Event Types

| Type | Meaning |
|------|---------|
| `"init"` | Rule applied during initial `ProcessAllBusinessRules()` on form mount. |
...
function MyForm() {
  ...
  return (
    <>
      ...
      <HookFormDevTools
        configName="myForm"
        configRules={configBusinessRules}
...
### Props (IHookFormDevToolsProps)

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `configName` | `string` | -- (required) | Display name shown in the DevTools header. |
| `configRules` | `IConfigBusinessRules` | `undefined` | The current config business rules (from `initBusinessRules()` or the provider). |
...
| Tab | Content |
|-----|---------|
| **Rules** | Lists every field with its current rule state: component, required, hidden, readOnly, validations, and valueFunction. |
...
| `unregistered_component` | The `component` type is not in the provided registered components set. Only checked when `registeredComponents` is provided. |
| `unregistered_validation` | A sync validation name in `validations[]` is not in the `ValidationRegistry`. |
| `unregistered_async_validation` | An async validation name in `asyncValidations[]` is not in the async `ValidationRegistry`. |
...
interface IConfigValidationError {
  type: "missing_dependency_target" | "unregistered_component" | "unregistered_validation"
      | "unregistered_async_validation" | "circular_dependency" | "missing_dropdown_options"
      | "self_dependency";
  fieldName: string;
  message: string;
  details?: string;
...
The dependency graph is automatically validated for cycles in dev mode when `GetDefaultBusinessRules()` is called (which happens during form initialization). If cycles are detected, warnings are logged to the console:
...
Check that the `dropdownDependencies` structure matches the trigger field's value exactly:

```typescript
// Trigger field: "category" with value "Engineering"
// This must match the key in dropdownDependencies:
dropdownDependencies: {
  "Engineering": {                  // Must match String(fieldValue) exactly
...
Also verify that:
- The target field name in `dropdownDependencies` matches an actual field in the config.
- The option keys in the string array match actual `key` values in the dropdown options.
...
Verify that:
- All fields listed in `dependencyRules.rules` exist in the config.
...
- The field has `orderDependencies` defined.
- The field's value matches a key in the `OrderDependencyMap`.
...
### "isReadonly deprecated warning"

The `isReadonly` property is deprecated in favor of `readOnly`. The engine normalizes it automatically but logs a warning in dev mode:

```
[dynamic-forms] Field "myField": "isReadonly" is deprecated. Use "readOnly" instead.
```

To fix: Rename `isReadonly` to `readOnly` in your field configs.
...
2. **Open DevTools** -- Add `<HookFormDevTools>` to visualize rules, values, errors, and the dependency graph.
...
6. **Check combo conditions** -- All `dependencyRules.rules` entries must match simultaneously (AND logic).
7. **Check dropdown keys** -- Option keys in `dropdownDependencies` arrays must match actual `IDropdownOption.key` values.
